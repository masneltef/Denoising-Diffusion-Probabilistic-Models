...
class DDPMScheduler(nn.Module):
    ...
    def set_timesteps(
        ...
        timesteps = np.arange(self.num_train_timesteps - 1, -1, -step)
    ...
    def previous_timestep(self, timestep):
        ...
        idx = (timesteps == timestep).nonzero(as_tuple=True)[0].item()
        if idx + 1 < len(timesteps):
            prev_t = timesteps[idx + 1].item()
        else:
            prev_t = 0
        return prev_t
    ...
    def _get_variance(self, t):
        ...
        alpha_prod_t_prev = self.alphas_cumprod[prev_t] if t > 0 else torch.tensor(1.0, device=alpha_prod_t.device, dtype=alpha_prod_t.dtype)
    ...
    def add_noise(
        ...
        sqrt_one_minus_alpha_prod = torch.sqrt(1.0 - self.alphas_cumprod[timesteps]).to(original_samples.device)
    ...
    def step(
        ...
        alpha_prod_t = self.alphas_cumprod[t]
        alpha_prod_t_prev = self.alphas_cumprod[prev_t] if t > 0 else torch.tensor(1.0, device=sample.device, dtype=sample.dtype)
        ...
        pred_original_sample_coeff = (
            torch.sqrt(alpha_prod_t_prev) * current_beta_t / (1.0 - alpha_prod_t)
        )
        current_sample_coeff = (
            torch.sqrt(current_alpha_t) * beta_prod_t_prev / (1.0 - alpha_prod_t)
        )
